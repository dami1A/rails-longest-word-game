...
class GamesController < ApplicationController
  ...
  def score
    ...
    @executive_test["found"] ? @result[:score] = 1 / @result[:time] + @word.length.to_f : @result[:score] = 0
  ...
  def word_testeur(word, grid)
    ...
    if first_test == true
      ...
      final_test = { "found" => false, "word" => word, "error" => "Sorry but '#{word.upcase}' can be built out of #{grid}" }
    end
    return final_test
  end
end
